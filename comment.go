...
// Comment is the database structure for the comments table
type Comment struct {
	ID     int
	UserID int
	PostID int
...
// InsertComment is used for the createComment mutation call
...
// RemoveCommentByID is used for the removeComment mutation call
func RemoveCommentByID(id int) error {
	_, err := db.Exec("DELETE FROM comments WHERE id=$1", id)
	return err
}

// GetCommentByIDAndPost is used for the comment query call
...
// GetCommentsForPost is used for the comment query call
